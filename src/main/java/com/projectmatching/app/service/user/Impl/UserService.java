...
public class UserService {
    ...
    //좋아요 누르기
    @Transactional
    public void addLiking(UserDetailsImpl userDetails, long userId) {
        ...
        userLikingRepository.save(userLikingDto.asEntity(from, to)).getId();


    }


    public void cancelUserLiking(UserDetailsImpl userDetails, Long targetUserId) {
        User user = userRepository.findByEmail(userDetails.getUserRealName()).orElseThrow(CoNectNotFoundException::new);
        UserLiking userLiking = userLikingRepository.findByUserAndTargetUser(user.getId(), targetUserId).orElseThrow(CoNectLogicalException::new);
        userLikingRepository.delete(userLiking);
    }
